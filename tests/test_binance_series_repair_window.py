...
def test_get_earliest_series_timestamp_returns_none_for_empty_table(db_session):
    assert get_earliest_series_timestamp('BTCUSDT', 'klines', session=db_session) is None
...
def test_get_earliest_series_timestamp_uses_kline_open_time(db_session):
    upsert_series_records(
        'klines',
        [
            {
    ...
    assert get_earliest_series_timestamp('BTCUSDT', 'klines', session=db_session) == 300000
...
def test_get_earliest_series_timestamp_uses_event_time_for_ratio_series(db_session):
    upsert_series_records(
        'top_long_short_position_ratio',
        [
            {
