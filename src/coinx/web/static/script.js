...
function fetchJsonWithTimeout(url, options = {}, timeoutMs = REFRESH_TIMEOUT_MS) {
  if (window.CoinxApi?.requestJson) {
    return window.CoinxApi.requestJson(url, {
      ...options,
      coinxTimeoutMs: timeoutMs,
      coinxSuppressGlobalError: true,
    });
  }
  ...
  fetchJsonWithTimeout("/api/update?force=true&wait=true")
    .then((updateResult) => {
      if (updateResult.status === "success" || updateResult.code === "COLLECTION_SCHEDULER_ONLY") {
        // 更新成功后再获取数据
        return fetchJsonWithTimeout("/api/coins?wait=true&nocache=1");
      } else {
