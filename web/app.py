...
def get_coins():
    ...
    try:
        ...
        for coin in coins_data:
                    ...
                    changes.append(
                        {
                            ...
                            "open_interest_value_formatted": data[
            ...
            changes.sort(
                key=lambda x: (
                    x["interval"].endswith("m")
                    and int(x["interval"][:-1])
                    or x["interval"].endswith("h")
                    and int(x["interval"][:-1]) * 60
                    or x["interval"].endswith("d")
                    and int(x["interval"][:-1]) * 1440
                    or 0
                )
            )
        ...
        logger.info(
            f"格式化后的数据示例: {formatted_data[:2] if formatted_data else '无数据'}"
        )
        ...
        error_response = {"status": "error", "message": f"获取币种数据失败: {str(e)}"}
...
def update_data():
    ...
    try:
        ...
        if not should_update_cache():
            response_data = {"status": "success", "message": "数据已是最新，无需更新"}
            logger.info("数据已是最新，无需更新")
            return jsonify(response_data)

        # 在后台线程中执行数据更新，不阻塞Web请求
        update_thread = threading.Thread(
            target=update_all_data, kwargs={"force_update": True}
        )
        ...
        error_response = {"status": "error", "message": f"数据更新失败: {str(e)}"}
...
def get_coins_config():
    ...
    try:
        ...
        error_response = {"status": "error", "message": f"获取币种配置失败: {str(e)}"}
        logger.error(f"获取币种配置失败: {e}")
        return jsonify(error_response), 500


@app.route("/api/coins-config", methods=["POST"])
def update_coin_tracking():
    """更新币种跟踪状态"""
    logger.info("更新币种配置")
    try:
        ...
        if not symbol:
            error_response = {"status": "error", "message": "缺少symbol参数"}
            logger.error("更新币种配置失败: 缺少symbol参数")
            return jsonify(error_response), 400

        set_coin_tracking(symbol, tracked)

        response_data = {"status": "success", "message": "配置更新成功"}
        logger.info(f"配置更新响应: {response_data}")
        return jsonify(response_data)
    except Exception as e:
        error_response = {"status": "error", "message": f"更新币种配置失败: {str(e)}"}
...
def add_coin_api():
    ...
    try:
        ...
        response_data = {"status": "success", "message": "币种添加成功"}
        logger.info(f"添加币种响应: {response_data}")
        return jsonify(response_data)
    except Exception as e:
        error_response = {"status": "error", "message": f"添加币种失败: {str(e)}"}
...
def delete_coin():
    ...
    try:
        ...
        response_data = {"status": "success", "message": "币种删除成功"}
        logger.info(f"删除币种响应: {response_data}")
        return jsonify(response_data)
    except Exception as e:
        error_response = {"status": "error", "message": f"删除币种失败: {str(e)}"}
...
def set_coin_track():
    ...
    try:
        ...
        response_data = {"status": "success", "message": "跟踪状态设置成功"}
    ...
    try:
        ...
        if success:
            ...
            try:
                ...
                response_data = {
                    "status": "success",
                    "message": "币种配置更新成功，正在拉取最新数据...",
                }
                logger.info("币种配置更新成功，已触发数据更新")
                return jsonify(response_data)
            except Exception as e:
                logger.warning(f"触发数据更新时出错: {e}")
                logger.exception(e)
                response_data = {
                    "status": "success",
                    "message": "币种配置更新成功，但数据更新触发失败",
                }
                return jsonify(response_data)
        else:
            error_response = {"status": "error", "message": "币种配置更新失败"}
