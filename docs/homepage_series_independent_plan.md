...
这份文档记录的是旧的“独立时间源”分析背景。当前实现已经切换到统一的最新已收盘 5m 锚点：

- 统一锚点名：`latest_closed_5m_open_time`
- 首页“当前值”和 `5m` 列显示的都是上一根已完整收盘的 5m candle
- `cache_update_time` 也显示这根已收盘 candle 的 open_time
- 倒计时面向下一个 5m 收盘点，而不是当前分钟

也就是说，首页现在优先保证一致性和可解释性，允许比真实时钟落后一根完整 5m，避免把正在形成的 candle 混入价格、持仓量和净流入计算。
...
| 持仓 | 快照 (当前 - 168h前) | ✓ |
| 价格 | 快照 (当前 - 168h前) | ✓ |
| net_inflow | **累计** (168h内所有点净流入，按 `buy_vol - sell_vol`) | ✓ 当前已正确 |
